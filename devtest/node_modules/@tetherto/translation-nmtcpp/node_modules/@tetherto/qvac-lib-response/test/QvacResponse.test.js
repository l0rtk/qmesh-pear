...
test('cancel throws error if response is finished', async t => {
  const response = new QvacResponse({
    cancelHandler: dummyCancelHandler,
    pauseHandler: dummyPauseHandler,
    continueHandler: dummyContinueHandler
  ...
  try {
    await response.cancel()
    t.fail('cancel should throw an error when not running')
  } catch (err) {
    t.is(
      err.message,
      'ERR_ALREADY_FINISHED',
      'cancel threw the correct error message'
    )
  }
})
