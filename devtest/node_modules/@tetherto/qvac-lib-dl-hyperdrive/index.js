...
const BaseDL = require('@tetherto/qvac-lib-dl-base')
...
const ProgressReport = require('@tetherto/infer-base/src/utils/progressReport')
...
class HyperDriveDL extends BaseDL {
  ...
  constructor (opts) {
    ...
    if (!drive) {
      if (!key) {
        throw new QvacErrorHyperdrive(ERR_CODES.KEY_OR_DRIVE_REQUIRED)
      }
      ...
      if (!key.startsWith(this.opts.prefix)) {
        ...
        throw new QvacErrorHyperdrive(ERR_CODES.KEY_INVALID, 'Key must start with ' + this.opts.prefix)
      }
    }
  ...
  _validateAndDecodeKey (key) {
    ...
    try {
      ...
      throw new QvacErrorHyperdrive(ERR_CODES.KEY_INVALID, err.message)
    }
  }
  ...
  async _open () {
    try {
      ...
      try {
        ...
        throw new QvacErrorHyperdrive(ERR_CODES.DRIVE_NOT_READY)
      }
      ...
      throw new QvacErrorHyperdrive(ERR_CODES.CONNECTION_FAILED)
    }
  }
  ...
  async getStream (filePath, opts = {}) {
    try {
      ...
      throw new QvacErrorHyperdrive(ERR_CODES.FILE_NOT_FOUND, `${filePath}: ${err.message}`)
    }
  }
  ...
  async getFileSize (path, opts = {}) {
    try {
      ...
      if (entry?.value?.blob) {
        ...
        throw new QvacErrorHyperdrive(ERR_CODES.FILE_NOT_FOUND, path)
      }
    } catch (err) {
