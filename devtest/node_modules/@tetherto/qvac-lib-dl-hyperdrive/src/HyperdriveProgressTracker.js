...
class HyperdriveProgressTracker {
    ...
    try {
      ...
      try {
        ...
        throw new QvacErrorHyperdrive(
          ...
          `Failed to access file: ${remotePath}`
        )
      }
          ...
          try {
            ...
            reject(
              new QvacErrorHyperdrive(
                ERR_CODES.DOWNLOAD_FAILED,
                `Failed to finalize file: ${err.message}`
              )
            )
          }
        })
      ...
      throw new QvacErrorHyperdrive(
        ERR_CODES.DOWNLOAD_FAILED,
        `Save to disk failed: ${err.message}`
      )
    }
  }
  ...
  async _downloadSingleFile (file) {
    ...
    if (!entry?.value?.blob) {
      ...
      throw new QvacErrorHyperdrive(ERR_CODES.FILE_NOT_FOUND, file)
    }
